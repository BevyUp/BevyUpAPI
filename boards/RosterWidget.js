...
"<div class='panel'>\
    <div class='panel-heading'>\
        <h4>User Identity</h4>\
        <button class='btn btn-info btn-sm logOutLink'>Log Out</div></h2>\
    </div>\
    <div class='panel-body content'>\
        <h2>Welcome, <span class='welcomeUserName'>" + localParticipant.getName() + "</span>!</>\
        <button class='btn btn-info btn-sm  dashboardLink' onclick='window.location.href = \'http://agent.bevyup.com\''>Dashboard</a>\
        <button class='btn btn-info btn-sm smsButton'>Send SMS</button>\
    </div>\
</div>";
...
<div>Link: " + window.location.origin + window.location.pathname + "?overrideInviteId=" + remoteParticipant.getInviteId() + "</div>";
            ...
            if (localParticipant.getIsAgent()) {
                ...
                remotePartHtml +=
"<button class='remotePartEdit btn btn-info btn-sm'>Edit</button>\
