(function ($) {
    ...
    function initCallback(isInSession) {
        ...
        .then(function (productListsModel) {
            // Get all the lists in the session and add it to the tagBoard
            tagBoard.updateLists(productListsModel);

            // Get the product list we want to use for the TagBoard, if it exists.
            var defaultList = productListsModel.getProductList("Board");
    ...
    function TagBoard(parent) {
        ...
        this.productCountHeader = container.querySelector('.title');
        this.scanButton = container.querySelector('.scan');
        this.listSelector = container.querySelector('.listSelector');
...
<h2 class='counter'>\
    ...
    <select class='listSelector'>\
        <option value='volvo'>(NO LISTS)</option>\
    </select>\
</h2>\
<ul class='tbw_list'></ul>";
    ...
    TagBoard.prototype.updateLists = function TagBoard_updatelists (productListsModel) {
        var self = this;
        ...
        // Add new ones
        var lists = productListsModel.getProductLists();
        lists.forEach(function (l) {
            var elem = document.createElement('option');
            elem.value = l.getName();
            elem.innerText = l.getName();
            self.listSelector.appendChild(elem);

            $(self.listSelector).change(function() {
                var list = productListsModel.getProductList($(self.listSelector).val());
                self.hookUpToList(list);
            });
        });

        // Select the first list
        var list = productListsModel.getProductList($(this.listSelector).val());
        this.hookUpToList(list);
    }
    ...
    TagBoard.prototype.spawnProductView = function TagBoard_spawnProductView(product) {
        ...
        $(this.productsList).prepend(productView.element);
    }
...
<div class='tbw_prodInfo'>\
    <div class='tbw_prodName'>" + productData.title + "</div>\
    <div class='tbw_prodPrice'>" + productData.brand + "</div>\
    ...
    <div class='tbw_prodCommentCount'></div>\
            ...
            var image = container.querySelector('.tbw_prodImg');
            image.style.backgroundImage = 'url("http://g.nordstromimage.com/imagegallery/store/product/Large' + productData.image + '")';
            var alt = JSON.stringify(productData);
            image.setAttribute("title", alt.replace('"', "'"));
