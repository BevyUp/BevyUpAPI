(function ($) {
    ...
    window.bevyUpPartnerAsyncInit = function () {
        if (overrideInviteId) {
            BevyUpApi.init(overrideInviteId).then(initCallback, initCallbackFailure);
        }
        else {
            BevyUpApi.init().then(initCallback, initCallbackFailure);
        }
    };
    ...
    function initCallback(sessionFound) {
        if (!sessionFound) {
            if (overrideInviteId) {
    ...
    function initCallbackFailure(err) {
        log("(error) initCallbackFailure: " + err);
    }
